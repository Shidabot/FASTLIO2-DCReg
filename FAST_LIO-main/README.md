...
### DCReg degeneracy handling in this fork

This fork integrates DCReg-style decoupled degeneracy characterization into
the FAST-LIO iterated error-state Kalman filter. For the first measurement
iteration of every LiDAR frame it:

1. extracts the pose information block in FAST-LIO order `[t, r]`;
2. uses Schur complements to eliminate translation and rotation separately;
3. detects weak eigen-directions in the two decoupled motion subspaces;
4. maps every eigen-direction to X/Y/Z axis energy shares for diagnostics;
5. applies the paper's Theorem-4 MAP eigenvalue clamping only in weak Schur
   directions, while retaining the original rotation-translation cross terms.
...
The paper recommends setting `dcreg_condition_threshold` and
`dcreg_kappa_target` to the same value, normally `10`.
`dcreg_regularization_alpha=1.0` applies the full MAP spectral clamp; lower
values soften the correction. The FAST-LIO information matrix is solved with
LDLT rather than a six-DoF PCG approximation, preserving EKF consistency.
Logs report rotation/translation Schur condition numbers, eigenvalues, and the
six weak subspace directions.
